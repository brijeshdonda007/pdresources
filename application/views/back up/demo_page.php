...
<div>
    ...
    <div align="center" id="cartInfo">
    	<h4>Shopping Cart</h4>
        <a href="javascript:void(0);" onclick="javascript:ShowhideCartContent()"><?php echo count($Basket['Course']);?>Item(s)-<?php echo ' Total= '.$Basket['Total'];?><?php if($Basket['Dicount_amt'] > 0) echo '<br>Discount amount is '.$Basket['Dicount_amt'].' and <br>subtotal is '.$Basket['SubTotal'];?></a>
...
<?php
	if(is_array($Basket['CoupenInfo']))
	{
		echo '<a href="'.site_url('cart_demo/RemoveCoupen').'">Delete Coupen</a>';
	}
	else
	{
		if($Basket['Coupen_error'])
			echo $Basket['Coupen_error'];
?>
        <form name="dicount" id="dicount" method="post">
            <label>Enter Coupen Code:</label><input type="text" name="coupon_code" id="coupon_code" />
            <input type="submit" name="submit" id="submit" value="Apply Coupon" />
        </form>
<?php
	}
?>
