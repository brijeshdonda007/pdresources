...
function unBlockUI(){
	$('#user_dashbord_image').attr('src','<?php echo $UserInfo['cust_avatar'];?>');
	$.unblockUI();
...
window.fbAsyncInit = function() {
	FB.init({appId: '377294372316766', status: true, cookie: true, xfbml: true,oauth : true});
	isLoaded = true;   
};
                ...
                <div class="active_course_box">
                    ...
                    if($ActiveCourseInfo)
                    {
                        foreach($ActiveCourseInfo as $key=>$val)
                        {
							...
							else if($val['cur_responce']['responce_status'] == 'Failed')
