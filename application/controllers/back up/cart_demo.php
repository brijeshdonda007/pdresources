...
class cart_demo extends CI_Controller
{
	...
	{
		$this->mdl_cart->RemoveCoupen();		
		redirect('cart_demo/index');
	}
	function SetData($action='')
	{		
		$Basket=$this->session->userdata('Basket');
		
		if($Basket['Dicount_amt'] > 0) 
			$extra='<br>Discount amount is '.$Basket['Dicount_amt'].' and <br>subtotal is '.$Basket['SubTotal'];
		echo '<div align="center" id="cartInfo">
    	<h4>Shopping Cart</h4>
        <a href="javascript:void(0);" onclick="javascript:ShowhideCartContent()">'.count($Basket['Course']).' Item(s)- Total= '.$Basket['Total'].$extra.'</a>';
		if($action == 'add')
			echo'<div id="cart_content" style="display:none;">';
		else
			echo'<div id="cart_content">';		
