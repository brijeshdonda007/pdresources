...
<div class="root_address">
  ...
  <a href="<?php echo site_url('admin/evaluation/index/'); ?>" class="root_link">Evaluation List</a>
  <span class="root_vr">/</span><span class="root_link_remove">Evaluation Question List</span>
  <div class="clear"></div>
</div>
<!--End [ Root Link ]-->
...
<div class="page">
  ...
  <div class="right_nav" style="width:98%">
    ...
    <div class="page_title">Evaluation Question List</div>
    ...
    <form name="delete_form" id="delete_form" action="<?php echo site_url('admin/evaluation/deleteQuest/'.$evaluation_id.'/'); ?>" method="post" onsubmit="return giveNotation();" >
     
     <div class="page_btn_space_right">
      <div style="float:left; padding-top:5px;">
        <a href="<?php echo site_url('admin/evaluation/addQuest/'.$evaluation_id); ?>" class="page_btn">Add</a>
            ...
            foreach($listArr as $list) {
			  ...
			  echo '<td>';
				if($i != 1)	  {  if($i==$num) $style = "margin-left:20px; "; ?>
                    <a href="<?php echo site_url('admin/evaluation/quest_sort/'.$evaluation_id.'/'.$list['question_order'].'/up'); ?>" style="<?php echo $style; ?> text-decoration:none;" title="Move Up" >
                        <img src="<?php echo base_url(); ?>images/admin/up_arrow.png" alt="Move Up" width="20" height="20" border="0" />
                    </a>
				<?php }  if($i != $num) { ?>
                    <a href="<?php echo site_url('admin/evaluation/quest_sort/'.$evaluation_id.'/'.$list['question_order'].'/down'); ?>" style="margin-left:20px; text-decoration:none;" title="Move Down" >
                        <img src="<?php echo base_url(); ?>images/admin/down_arrow.png" alt="Move Down" width="20" height="20" border="0" />
                    </a>
				<?php }
				echo '</td>';			  			  
              echo '<td><a href="'.site_url('admin/evaluation/addQuest/'.$evaluation_id.'/'.$list['question_id']).'"> Edit </a> | ';
              if($list['is_active'] == "Y") {
	          	echo '<a href="javascript:activeQuest(\''.$list['question_id'].'\',\'N\',this)" id="link_'.$list['question_id'].'" title="Inactivate Evaluation Question">Active </a> | ';
			  }
			  else
				echo '<a href="javascript:activeQuest(\''.$list['question_id'].'\',\'Y\',this)" id="link_'.$list['question_id'].'" title="Acviate Evaluation Question">In-active </a> | ';
              echo '<a href="javascript:deleteQuest('.$evaluation_id.','.$list['question_id'].')"> Delete </a> |';
              echo '<a href="'.site_url('admin/evaluation/option/'.$evaluation_id.'/'.$list['question_id']).'"> Add Options </a></td>';			  
              echo '</tr>';
			  $i++;
            }
			if(count($listArr) == 0)
...
<script type="text/javascript">
	...
	function deleteQuest(evaluation_id,id)
	{
		...
		{
			$.ajax({
				url: "<?php echo site_url('admin/evaluation/deleteQuest/'); ?>/"+evaluation_id+"/"+id,
